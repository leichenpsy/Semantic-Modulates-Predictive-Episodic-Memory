...
## Compile data files
setwd('/Users/leichen/Research project/Predictive memory/Phonologyv2 Data (2022.04.07 - 2022.04.14)')
n_subject <- length(list.files('/Users/leichen/Research project/Predictive memory/Phonologyv2 Data (2022.04.07 - 2022.04.14)'))
data_all <- list.files(path = '/Users/leichen/Research project/Predictive memory/Phonologyv2 Data (2022.04.07 - 2022.04.14)') %>%
  lapply(read.csv) %>%
  bind_rows()
subjectExcluded <- c()
...
encodeData <- encodeData[,coln_trans] %>%
  ...
  mutate(preCaType = ifelse(category == lag(category),1,0)) %>%
  mutate(postCaType = ifelse(category == lead(category),1,0)) %>%
  mutate(recode = case_when(preCaType == 1 & postCaType == 1 ~ 'same-item-same', preCaType == 1 & postCaType == 0 ~ 'same-item-diff', preCaType == 0 & postCaType == 1 ~ 'diff-item-same', preCaType == 0 & postCaType == 0 ~ 'diff-item-diff')) %>%
  ungroup()
encodeData[,26:28][is.na(encodeData[,26:28])] <- 0
...
quartile <- encodeData[,c('subject','currTrial','stimulus','repFiller','preCaType','postCaType', 'recode')] %>%
  group_by('subject') %>%
  arrange('currTrial') %>%
...
memoryData <- merge(memoryData,quartile[,c('subject','q','stimulus', 'half','repFiller','preCaType','postCaType','recode')],by = c('subject','stimulus'),all = TRUE)
...
memoryPrecision <- memoryCount %>%
  ...
  summarise(meanPrecision = mean(precision))
  
memoryPrecisionModel <- lmer(meanPrecision ~ type * predictType + (1|subject), data = memoryPrecision)

anova(memoryPrecisionModel)
summary(memoryPrecisionModel)

ggboxplot(memoryPrecision, x = 'predictType', y = 'meanPrecision', color = 'type', add = 'jitter')
ggbarplot(memoryPrecision, x = 'predictType', y = 'meanPrecision', fill = 'type', add = 'mean_se', position = position_dodge(0.8)) #+ geom_jitter(aes(x = type, y = precision, fill = 'predictType' ), color = 'darkgrey', alpha = 0.65, position = position_jitterdodge(jitter.height = .1, jitter.width = .2))

##### The last 3 quartiles #####
...
######## Filter out repetitive fillers ########

####### All Trials ######

memoryCount <- memoryTest %>%
  filter(repFiller != 1 || is.na(repFiller)) %>%
  group_by(subject, type, predictType) %>%
  filter(!subject %in% subjectExcluded)%>%
  count(sdt) %>%
  pivot_wider(names_from = 'sdt', names_prefix = 'n_', values_from = 'n')
...
### Calculate all trials + all times ###
...
memoryHitRateModel <- lmer(hit_rate ~ type * predictType + (1|subject), data = memoryHitRate)
anova(memoryHitRateModel2)
summary(memoryHitRateModel2)

emHit <- emmeans(memoryHitRateModel, ~type * predictType)
contrast(emHit, 'pairwise', by = 'predictType')
contrast(emHit, 'pairwise', by = 'type')

hitFlat <- memoryHitRate %>%
  pivot_wider(id_cols = 'subject', names_from = c('type','predictType'), values_from = 'hit_rate')
...
ggplot(memoryHitRate, aes(x = type, y = hit_rate, color = predictType)) + geom_boxplot()
ggboxplot(memoryHitRate, x = 'predictType', y = 'hit_rate', fill = 'type') + geom_jitter(aes(x = predictType, y = hit_rate, fill = type), color = 'grey', alpha = 0.9, position = position_jitterdodge(jitter.width = 0.2))
p <- ggbarplot(memoryHitRate, x = 'predictType', y = 'hit_rate', fill = 'type', add = 'mean_se', position = position_dodge(0.8))# + geom_jitter(aes(x = type, y = sensitivity, fill = predictType), color = 'grey', alpha = 0.9, position = position_jitterdodge(jitter.width = 0.2, jitter.height = 0.1))
...
memoryHitQ <- memoryHitQ %>%
  ...
  filter(!subject %in% subjectExcluded)%>%
  mutate(hit_rate = n_hit/(n_hit + n_miss))
memoryHitQModel <- lmer(hit_rate ~ q * type * predictType + (1|subject), data = memoryHitQ)
anova(memoryHitQModel)
ggbarplot(memoryHitQ, x = 'type', y = 'hit_rate', fill = 'predictType', add = 'mean_se', position = position_dodge(0.8), facet.by = 'q', panel.labs = list(q = c ('1st quartile', '2nd quartile', '3rd quartile', '4th quartile')))
...
memoryHitH <- memoryHitH %>%
  ...
  filter(!subject %in% subjectExcluded)%>%
  mutate(hit_rate = n_hit/(n_hit + n_miss))
memoryHitHModel <- lmer(hit_rate ~ half * type * predictType + (1|subject), data = memoryHitH)
...
### calculate hit rate + last 3 quartiles ###

memoryHit3Q <- memoryData %>%
  filter(response != 'no response detected') %>% #filter out no response trials
  filter(q != 1 || is.na(q)) %>%
  mutate(sdt = case_when(picType == 'foil' & acc == 1 ~ 'cr', picType == 'foil' & acc == 0 ~ 'fa', picType == 'old' & acc == 1 ~ 'hit', picType == 'old' & acc == 0 ~ 'miss') ) %>%
  group_by(subject, type, predictType) %>%
  count(sdt) %>%
  pivot_wider(names_from = 'sdt', names_prefix = 'n_', values_from = 'n')
memoryHit3Q[,4:7][is.na(memoryHit3Q[,4:7])]<-0 

memoryHit3Q <- memoryHit3Q %>%
  group_by(subject, type, predictType) %>%
  mutate(hit_rate = n_hit/(n_hit + n_miss))

memoryHit3QModel <- lmer(hit_rate ~ type * predictType + (1|subject), data = memoryHit3Q)
anova(memoryHit3QModel)

p <- ggbarplot(memoryHit3Q, x = 'predictType', y = 'hit_rate', fill = 'type', add = 'mean_se', position = position_dodge(0.8))
ggpar(p, ylim = c(0,0.5)) + scale_y_continuous(breaks = get_breaks(n = 6))
...
########## calculating dprime using psycho ##########
...
aprimeModel <- lmer(aprime ~ type * predictType + (1|subject), data = dprimeMemory)
anova(aprimeModel)


dprimeModel <- lmer(dprime ~ type * predictType + (1|subject), data = dprimeMemory)
anova(dprimeModel)
...
memoryPrep <- memoryCount %>%
  mutate(hit =  n_hit/(n_hit + n_miss), fa = n_fa/(n_cr + n_fa))%>%
  mutate(zHit = qnorm(hit), zFa = qnorm(fa)) %>% 
  mutate(dprime = zHit - zFa)

df <- data.frame(n_hit = c(10, 21, 18, 12), n_miss = c(12, 20, 19, 23), n_cr = c (13, 8, 19, 21), n_fa = c(15, 12, 7, 19))
testIndices <- psycho::dprime(n_hit = df$n_hit, n_cr = df$n_cr, n_miss = df$n_miss, n_fa = df$n_fa, adjusted = F)
dfDprime <- df %>%
  mutate(hit_rate = n_hit /(n_hit + n_miss), fa_rate = n_fa/(n_fa + n_cr)) %>%
  mutate(zHit = qnorm(hit_rate), zFa = qnorm(fa_rate)) %>%
  mutate(dprime = zHit - zFa)
...
memoryFaRateModel <- lmer(fa_rate ~ type * predictType + (1|subject), data = memoryFaRate)
anova(memoryFaRateModel)
emFa <- emmeans(memoryFaRateModel, ~type * predictType)
...
summary(coverTaskAccModel)

ggboxplot(coverTaskAcc, x = 'predictType', y = 'acc', fill = 'type') + geom_jitter(aes(x = predictType, y = acc, fill = type), color = 'grey', alpha = 0.9, position = position_jitterdodge(jitter.width = 0.2, jitter.height = 0.1))

p <- ggbarplot(coverTaskAcc, x = 'predictType', y = 'acc', fill = 'type', add = 'mean_se', position = position_dodge(0.8))#+ geom_jitter(aes(x = predictType, y = acc, fill = type), color = 'grey', alpha = 0.9, position = position_jitterdodge(jitter.width = 0.2, jitter.height = 0.1))
...
p <- ggbarplot(coverTaskAccH, x = 'predictType', y = 'acc', fill = 'type', add = 'mean_se', position = position_dodge(0.8), facet.by = 'half', panel.labs = list(half = c('1st half', '2nd half')))
ggpar(p, ylim = c(0,1)) + scale_y_continuous(breaks = get_breaks(n = 6))

emcoverTaskAccH <- emmeans(coverTaskAccHModel, ~half*predictType*type)
contrast(emcoverTaskAccH, 'pairwise', by = 'half')

## Calculate Acc + last 3 quartiles
coverTaskAcc3Q <- encodeData %>%
  filter(q != 1) %>%
  group_by(subject, type, predictType) %>%
  summarise(acc = mean(acc))
coverTaskAcc3QModel <- lmer(acc ~ type * predictType + (1|subject), data = coverTaskAcc3Q)
anova(coverTaskAcc3QModel)
summary(coverTaskAcc3QModel)
p <- ggbarplot(coverTaskAcc3Q, x = 'predictType', y = 'acc', fill = 'type', add = 'mean_se', position = position_dodge(0.8))#+ geom_jitter(aes(x = predictType, y = acc, fill = type), color = 'grey', alpha = 0.9, position = position_jitterdodge(jitter.width = 0.2, jitter.height = 0.1))
ggpar(p, ylim = c(0,1)) + scale_y_continuous(breaks = get_breaks(n = 6))
...
reEncodeDataAcc <- encodeData %>%
  filter(repFiller != 1) %>%
  group_by(subject, type, recode) %>%
  summary(acc = mean(acc))

reEncodeDataAccModel <- lmer(acc ~ type * recode + (1|subject), data = reEncodeDataAcc)
anova(reEncodeDataAccModel)
summary(reEncodeDataAccModel)

p <- ggbarplot(reEncodeDataAcc, x = 'recode', y = 'acc', fill = 'type', add = 'mean_se', position = position_dodge(0.8))#+ geom_jitter(aes(x = predictType, y = acc, fill = type), color = 'grey', alpha = 0.9, position = position_jitterdodge(jitter.width = 0.2, jitter.height = 0.1))
ggpar(p, ylim = c(0,1)) + scale_y_continuous(breaks = get_breaks(n = 6))

## Calculate ACC + Half
coverTaskAccH <- encodeData %>%
  filter(repFiller != 1) %>%
  group_by(subject, type, recode, half) %>%
  filter(!subject %in% subjectExcluded)%>%
  summarise(acc = mean(acc))

coverTaskAccHModel <- lmer(acc ~ type * recode * half + (1|subject), data = coverTaskAccH)
anova(coverTaskAccHModel)
summary(coverTaskAccHModel)

p <- ggbarplot(coverTaskAccH, x = 'recode', y = 'acc', fill = 'type', add = 'mean_se', position = position_dodge(0.8), facet.by = 'half', panel.labs = list(half = c('1st half', '2nd half'))) + rotate_x_text(45) 
ggpar(p, ylim = c(0,1)) + scale_y_continuous(breaks = get_breaks(n = 6))
...
coverTaskAcc3Q <- encodeData %>%
  ...
  group_by(subject, type, recode) %>%
  summarise(acc = mean(acc))
coverTaskAcc3QModel <- lmer(acc ~ type * recode + (1|subject), data = coverTaskAcc3Q)
anova(coverTaskAcc3QModel)
summary(coverTaskAcc3QModel)
p <- ggbarplot(coverTaskAcc3Q, x = 'recode', y = 'acc', fill = 'type', add = 'mean_se', position = position_dodge(0.8))#+ geom_jitter(aes(x = predictType, y = acc, fill = type), color = 'grey', alpha = 0.9, position = position_jitterdodge(jitter.width = 0.2, jitter.height = 0.1))
ggpar(p, ylim = c(0,1)) + scale_y_continuous(breaks = get_breaks(n = 6))
...
summary(coverTaskRTModel)

ggboxplot(coverTaskRT,x = 'predictType', y = 'rt', fill = 'type') + geom_jitter(aes(x = predictType, y = rt, fill = type), color = 'grey',alpha = 0.9, position = position_jitterdodge(jitter.width = 0.2, jitter.height = 0.1)) 

p <- ggbarplot(coverTaskRT, x = 'predictType', y = 'rt', fill = 'type', add = 'mean_se', position = position_dodge(0.8)) #+ geom_jitter(aes(x = predictType, y = rt, fill = type), color = 'grey', alpha = 0.9, position = position_jitterdodge(jitter.width = 0.2, jitter.height = 0.1))
ggpar(p, ylim = c(0,800)) + scale_y_continuous(breaks = get_breaks(n = 8))

emcoverTaskRT <- emmeans(coverTaskRTModel, ~ type * predictType)
contrast(emcoverTaskRT, 'pairwise', by = 'predictType')

coverTaskRTFlat <- coverTaskRT %>%
  pivot_wider(id_cols = 'subject', names_from = c('type','predictType'), values_from = 'rt')
t.test(coverTaskRTFlat$filler_NA,coverTaskRTFlat$target_within, paired = T)
t.test(coverTaskRTFlat$target_within,coverTaskRTFlat$cue_cross, paired = T)


## Calculate RT + all times + 4 quartiles
...
ggboxplot(coverTaskRTQ, x = 'predictType', y = 'rt', fill = 'type') + geom_jitter(aes(x = predictType, y = rt, fill = type), position = position_jitterdodge(jitter.width = 0.2), alpha = 0.8) + facet_wrap(~q)
ggbarplot(coverTaskRTQ, x = 'predictType', y = 'rt', fill = 'type', color = 'type', position = position_dodge(0.8)) + geom_jitter(aes(x = predictType, y = rt, fill = type), position = position_jitterdodge(jitter.width = 0.2), alpha = 0.8) + facet_wrap(~q)

coverTaskRTQPlot <- coverTaskRTQ %>%
...
ggplot(coverTaskRTQPlot, aes(x = q, y = rt, color = combo, group = combo)) + geom_point() + geom_line()
ggbarplot(coverTaskRTQ, x = 'type', y = 'rt', fill = 'predictType', add = 'mean_se', position = position_dodge(0.8), facet.by = 'q')
ggline(coverTaskRTQPlot, x = 'q', y = 'rt', color = 'combo', add = 'mean_se', position = position_dodge(0.1)) 
...
p <- ggbarplot(coverTaskRTH, x = 'predictType', y = 'rt', fill = 'type', facet.by = 'half', panel.labs = list(half = c('1st half', '2nd half')), add = 'mean_se', position = position_dodge(0.8))
ggpar(p, ylim = c(0,800)) + scale_y_continuous(breaks = get_breaks(n = 8))
...
## Calculate RT + Last 3 quartiles

coverTaskRT3Q <- encodeData %>%
  filter(response == correctResponse & q != 1) %>%
  mutate(rt = as.numeric(responseTime)) %>%
  group_by(subject, type, predictType) %>%
  filter(!subject %in% subjectExcluded)%>%
  summarise(rt = median(rt)) 

coverTaskRT3QModel <- lmer(rt ~ type * predictType + (1|subject), data = coverTaskRT3Q)
anova(coverTaskRT3QModel)
summary(coverTaskRT3QModel)

p <- ggbarplot(coverTaskRT3Q, x = 'predictType', y = 'rt', fill = 'type', add = 'mean_se', position = position_dodge(0.8)) #+ geom_jitter(aes(x = predictType, y = rt, fill = type), color = 'grey', alpha = 0.9, position = position_jitterdodge(jitter.width = 0.2, jitter.height = 0.1))
ggpar(p, ylim = c(0,800)) + scale_y_continuous(breaks = get_breaks(n = 8))
...
coverTaskRT <- encodeData %>%
  filter(response == correctResponse & repFiller != 1)%>%
  group_by(subject, type, predictType) %>%
  filter(!subject %in% subjectExcluded)%>%
  mutate(rt = as.numeric(responseTime)) %>%
  summarize(rt = median(rt))
...
coverTaskRT3Q <- encodeData %>%
  filter(response == correctResponse & q != 1 & repFiller != 1) %>%
  mutate(rt = as.numeric(responseTime)) %>%
  group_by(subject, type, predictType) %>%
  filter(!subject %in% subjectExcluded)%>%
  summarise(rt = median(rt)) 
...
coverTaskRTModel <- lmer(rt ~ type * recode + (1|subject), data = coverTaskRT)
anova(coverTaskRTModel)
summary(coverTaskRTModel)

ggboxplot(coverTaskRT,x = 'recode', y = 'rt', fill = 'type') + geom_jitter(aes(x = recode, y = rt, fill = type), color = 'grey',alpha = 0.9, position = position_jitterdodge(jitter.width = 0.2, jitter.height = 0.1)) 

p <- ggbarplot(coverTaskRT, x = 'recode', y = 'rt', fill = 'type', add = 'mean_se', position = position_dodge(0.8)) #+ geom_jitter(aes(x = predictType, y = rt, fill = type), color = 'grey', alpha = 0.9, position = position_jitterdodge(jitter.width = 0.2, jitter.height = 0.1))
ggpar(p, ylim = c(0,800)) + scale_y_continuous(breaks = get_breaks(n = 8))

emcoverTaskRT <- emmeans(coverTaskRTModel, ~ type * recode)
contrast(emcoverTaskRT, 'pairwise', by = 'recode')
...
coverTaskRTQ <- encodeData %>%
  filter(response == correctResponse & repFiller != 1) %>%
  mutate(rt = as.numeric(responseTime)) %>%
  group_by(subject, type, recode, q) %>%
  filter(!subject %in% subjectExcluded)%>%
  summarise(rt = median(rt))

ggboxplot(coverTaskRTQ, x = 'recode', y = 'rt', fill = 'type') + geom_jitter(aes(x = recode, y = rt, fill = type), position = position_jitterdodge(jitter.width = 0.2), alpha = 0.8) + facet_wrap(~q)
ggbarplot(coverTaskRTQ, x = 'recode', y = 'rt', fill = 'type', color = 'type', position = position_dodge(0.8)) + geom_jitter(aes(x = recode, y = rt, fill = type), position = position_jitterdodge(jitter.width = 0.2), alpha = 0.8) + facet_wrap(~q)

coverTaskRTQPlot <- coverTaskRTQ %>%
  group_by(subject, recode, type, q) %>%
  summarise(rt = median(rt))%>%
  pivot_wider(names_from = c(type, recode), values_from = rt) %>%
  pivot_longer(cols = c(cue_within, cue_cross, target_within, target_cross, filler_NA), names_to = 'combo', values_to = 'rt') 
...
coverTaskRTHModel <- lmer(rt ~ type * recode*half + (1|subject), data = coverTaskRTH)
anova(coverTaskRTHModel)

p <- ggbarplot(coverTaskRTH, x = 'recode', y = 'rt', fill = 'type', facet.by = 'half', panel.labs = list(half = c('1st half', '2nd half')), add = 'mean_se', position = position_dodge(0.8)) + rotate_x_text(45) 
ggpar(p, ylim = c(0,900)) + scale_y_continuous(breaks = get_breaks(n = 8))

emcoverTaskRTH <- emmeans(coverTaskRTHModel, ~ type * recode * half)
contrast(emcoverTaskRTH,'pairwise', by = 'half')
...
coverTaskRT3QModel <- lmer(rt ~ type * recode + (1|subject), data = coverTaskRT3Q)
anova(coverTaskRT3QModel)
summary(coverTaskRT3QModel)

p <- ggbarplot(coverTaskRT3Q, x = 'recode', y = 'rt', fill = 'type', add = 'mean_se', position = position_dodge(0.8)) #+ geom_jitter(aes(x = predictType, y = rt, fill = type), color = 'grey', alpha = 0.9, position = position_jitterdodge(jitter.width = 0.2, jitter.height = 0.1))
ggpar(p, ylim = c(0,800)) + scale_y_continuous(breaks = get_breaks(n = 8))
...
ggboxplot(predictResult, x = 'scrambleType', y = 'meanAcc', fill = 'pairCaType') + geom_jitter(aes(x = scrambleType, y = meanAcc, fill = pairCaType), color = 'grey', position = position_jitterdodge(jitter.width = 0.2), alpha = 0.8)
...
predictTestChance <- predictResult %>%
  filter(scrambleType == 'swapping'& pairCaType == 'across')
