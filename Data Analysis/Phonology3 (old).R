...
ggboxplot(coverTaskAccPredictType, x = 'predictType', y = 'acc', fill = 'type') + geom_jitter(aes(x = predictType, y = acc, fill = type), color = 'grey', alpha = 0.9, position = position_jitterdodge(jitter.width = 0.2, jitter.height = 0.1))
...
coverTaskAccPredictTypeModel1 <- lmer(acc~type + predictType + type * predictType + (1|subject), data = coverTaskAccPredictType)
...
emcoverTaskAccH <- emmeans(coverTaskAccHModel1, ~half*predictType)
...
coverTaskRTPredictType <- encodeData %>%
  filter(response == correctResponse & repFiller == 0)%>%
  group_by(subject, type, predictType) %>%
  filter(!subject %in% subjectExcluded)%>%
  mutate(rt = as.numeric(responseTime)) %>%
  summarize(rt = median(rt))
...
coverTaskRTFlat <- coverTaskRTPredictType %>%
...
ggpar(p, ylim = c(0,850)) + scale_y_continuous(breaks = get_breaks(n = 8))
